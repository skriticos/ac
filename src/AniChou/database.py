...
class db(object):
    ...
    def __init__(self):
        ...
        if not path.isfile(ac_data_path):
            ...
            print dbstring.has_table('anime_myanimelist')
            print "\n"
            if not dbstring.has_table('anime_myanimelist'):
                _create_table()
    ...
    def set_db(self, d):
        ...
        table_mal = Table('anime_myanimelist', metadata, autoload=True)
...
def _create_table():
    ...
    dbstring = create_engine('sqlite:///' + ac_data_path)
    metadata = MetaData(dbstring)
    table_mal = Table('anime_myanimelist', metadata,
...
def _table_exists():
    """ see if the called table exists
    """
    dbstring = create_engine('sqlite:///' + ac_data_path)
    if not dbstring.has_table('anime_myanimelist'):
        _create_table()
