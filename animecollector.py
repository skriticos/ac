...
class leeroyjenkins:
    ...
    def __init__(self):
        ...
        for groups in self.config.optionGroups:
            if groups[2]:
                self.preferenceGroups.append(groups)
    ...
    def setPrefs(self):
        ...
        for prefs in self.preferenceGroups:
            for (name, group) in prefs[0].iteritems():
                for (item, value) in group.iteritems():
...
class config:
    ...
    def __init__(self):
        ...
        self.optionGroups = [[self.connection, "connection", True], 
                             [self.list, "list", True], 
                             [self.ui, "ui", True], 
                             [self.options, "options", False]]

        for opts in self.optionGroups:
